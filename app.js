...
(function(){
  ...
  async function loadVanilla(target){
    try {
      const base = 'assets/vanilla-tables/';
      const groups = [
        ['default_loottable_aggregated_probability.json','default_loottable_probability.json'],
        ['default_bradley_loottable_aggregated_probability.json','default_bradley_loottable_probability.json'],
        ['default_heli_loottable_aggregated_probability.json','default_heli_loottable_probability.json']
      ];
      const set = state.sets[target];
      set.files = [];
      ...
      if (!state.activeContainer){ const order = set.containerOrder; state.activeContainer = order[0] || null; }
      renderFileLists();
      render();
    } catch (e) { console.warn('Failed loading vanilla tables', e); }
  ...
  function rebuildSet(set){
    ...
    set.containers = new Map();
    set.containerOrder = [];
    files.forEach(function(f){ mergeDataIntoSet(set, f.data); });
  }
  ...
  function renderFileLists(){
    ...
    if (listA){
      listA.innerHTML = '';
      state.sets.A.files.forEach(function(f){
    ...
    if (listB2){
      listB2.innerHTML = '';
      state.sets.B.files.forEach(function(f){
        const li = document.createElement('li');
        const span = document.createElement('span'); span.className = 'name'; span.textContent = f.name;
        const rm = document.createElement('span'); rm.className = 'remove'; rm.textContent = '×'; rm.title = 'Remove';
  ...
  // Hook container search after functions are defined
  if (containerSearch){ containerSearch.addEventListener("input", () => { state.containerFilter = (containerSearch.value||"").toLowerCase(); renderContainers(); renderTables(); }); }
  function renderContainers(){
    ...
    let list = Array.from(containers);
    const ft = (state.containerFilter || '').toLowerCase();
    if (ft){
      list = list.filter(c => {
        const bucket = state.sets.A.containers.get(c) || state.sets.B.containers.get(c) || targetSet.containers.get(c);
        const short = bucket?.shortname || '';
        return c.toLowerCase().includes(ft) || short.toLowerCase().includes(ft);
      });
    }
  ...
  function renderTables(){
    ...
    labels.appendChild(h); labels.appendChild(sub); containerMeta.appendChild(labels);
    ...
    if (!state.compare){
      ...
      const { table } = buildTableForBucket(bucket, (bucket.combos.length === 0), filterText);
  ...
  // Attach table search listener after renderTables is defined
  if (searchInput){ searchInput.addEventListener("input", () => renderTables()); }
  ...
  function buildTableForBucket(bucket, aggregated, filterText){
    ...
    if (filterText){
      ...
      rows = rows.filter(r => {
        const shortText = Array.isArray(r.short) ? r.short.join(' ') : (r.short || '');
        return r.name.toLowerCase().includes(ft) || shortText.toLowerCase().includes(ft);
      });
    }
    ...
    for (const r of rows){
      ...
      labels.appendChild(title); labels.appendChild(sub);
      nameCell.appendChild(labels); tdName.appendChild(nameCell);
