...
(function(){
  ...
  function createEmptySet(){
    return { containers: new Map(), containerOrder: [] };
  }

  fetch('assets/names.json')
    .then(r => r.ok ? r.json() : { items:{}, containers:{} })
    .catch(() => ({items:{}, containers:{}}))
    .then(n => { state.names = { items: n.items || {}, containers: n.containers || {} }; updateNamesStatus(); });
  ...
  if (namesFileInput) namesFileInput.addEventListener('change', onNamesFile);
  ...
  async function readJsonFileSafe(file){
    try { const text = await file.text(); return JSON.parse(text); } catch { return null; }
  }
  ...
  function renderTables(){
    ...
    if (!state.compare){
      ...
      const { table } = buildTableForBucket(bucket, showAggregated, filterText);
      ...
      const aTbl = buildTableForBucket(left, showAggregated, filterText).table; aWrap.appendChild(aTitle); aWrap.appendChild(aTbl);
      ...
      const bTbl = buildTableForBucket(right, showAggregated, filterText).table; bWrap.appendChild(bTitle); bWrap.appendChild(bTbl);
  ...
  function downloadUpdatedNames(){
    ...
    const a = document.createElement('a'); a.href = url; a.download = 'names.json'; document.body.appendChild(a); a.click(); a.remove(); URL.revokeObjectURL(url);
  }

  async function onNamesFile(e){
    const file = e.target.files?.[0]; if (!file) return;
    try{
      const text = await file.text(); const json = JSON.parse(text);
      state.names = { items: json.items || {}, containers: json.containers || {} };
      state.unknown.items.clear(); state.unknown.containers.clear();
      for (const set of Object.values(state.sets)){
        for (const c of set.containerOrder){ const bucket = set.containers.get(c); bucket?.aggregated?.forEach(a => ensureItemKnown(a.shortname)); bucket?.combos?.forEach(cb => cb.items.forEach(ensureItemKnown)); }
      }
      render();
